...
process.stdin.on("data", function (key) {
  ...
  if (key === ctrlc) {
    ...
    // ↑
    if (key === up) {
      device.setVolume(volume + 0.05, onVolume);
    }
    // ↓
    else if (key === down) {
      device.setVolume(volume - 0.05, onVolume);
...
function onSeek (err, status) {
  if (err) {
    console.error("Could not seek into video", err.message);
    debug(err.stack);
  } else {
    console.log("Seeked to: " + status.currentTime);
  }
}
